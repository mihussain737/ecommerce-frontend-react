...
const Cart = () => {
  ...
  if(!cart || cart.length===0) <h1>Cart is Empty</h1>
  ...
  return (
    <div className="lg:px-14 sm:px-8 px-4 py-10">
      ...
      <div className="grid md:grid grid-cols-5 gap-4 pb-2 font-semibold items-center">
      ...
      <div className="border-t-[1.5px] border-slate-200 py-4 flex sm:flex-row sm:px-0 px-2 flex-col sm:justify-between gap-4">
          ...
          <div className="flex text-sm gap-1 flex-col">
               ...
               <Link className="w-full flex justify-end" to="/checkout">
                    <button onClick={()=> {}}
                         className="font-semibold w-[300px] py-2 px-4 rounded-sm bg-blue-600 flex justify-center items-center gap-2
                         hover:text-gray-300 transition duration-500 text-white">
                          <MdShoppingCart size={20}/>    Checkout
                    </button>
               </Link>

               <Link className=" flex gap-2 items-center text-slate-500 mt-2" to="/products">
               <MdArrowBack/>
